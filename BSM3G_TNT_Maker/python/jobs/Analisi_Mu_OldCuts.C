...
for (Int_t i=0;i<a_->GetEntries();i++){ 
 ...
 if (Muon_pt->size() > 1 && BoostedJet_pt->size() > 0){
  if (numOfHighptMu==2 && Muon_pt->at(0) > 53 && Muon_pt->at(1) > 30 && fabs(Muon_eta->at(0))<2.4 && fabs(Muon_eta->at(1))<2.4 && M_leplep>300 && BoostedJet_pt->at(0)>190 && numOfVetoEle==0 && numOfBoostedJets>= 1 && HLT_Mu==1){
...
TFile *f = new TFile("OC_plot/OldCuts_Sign_mumu_L11_M7000.root", "RECREATE");
num->Write();
M_lljet->Write();
