...
a_->Add("/eos/user/v/vmariani/NTuples/HN/Sign_mumu_L20_M1000.root");
...
TH1D *M_ll_shape = new TH1D ("M_ll_shape", "M_ll_shape", 500, 500, 8000);
TH1D *Muon1_pt = new TH1D ("Muon1_pt", "Muon1_pt", 200, 250, 5000);
TH1D *Muon2_pt = new TH1D ("Muon2_pt", "Muon2_pt", 200, 100, 5000);
...
TH1D *BoostJet_pt = new TH1D ("BoostJet_pt", "BoostJet_pt", 200, 180, 5000);
...
int tot = 0;
double deltaR = 0, deltaEta = 0, deltaPhi = 0, deltaR_Mu12 = 0, deltaEta_Mu12 = 0, deltaPhi_Mu12 = 0, deltaR_Mu1J = 0, deltaEta_Mu1J = 0, deltaPhi_Mu1J = 0, deltaR_Mu2J = 0, deltaEta_Mu2J = 0, deltaPhi_Mu2J = 0;
double wg = 0;
double lumi = 41529;
for (Int_t i=0;i<a_->GetEntries();i++){ 
 ...
 if (Muon_pt->size() > 1 && BoostedJet_pt->size() > 0){
  if (numOfHighptMu==2 && Muon_pt->at(0) > 250 && Muon_pt->at(1) > 100 && fabs(Muon_eta->at(0))<2.4 && fabs(Muon_eta->at(1))<2.4 && M_leplep>500 && BoostedJet_pt->at(0)>190 && numOfVetoEle==0 && numOfBoostedJets>= 1 && HLT_Mu==1){
...
TFile *f = new TFile("LM_plot/LowMass_Selection_Sign_mumu_L20_M1000.root", "RECREATE");
num->Write();
Mu2J_deltaR->Write();
Mu1J_deltaR->Write();
Mu12_deltaR->Write();
